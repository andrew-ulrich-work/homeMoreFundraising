...
var saveBotMessageToThread = function(phone, message, cb) {
  ...
  User.findOne({phone: phone}).exec(function(err, user) {
    ...
    // If not, create the user with that phone number
      ...
      User.create({type: 'client', phone: phone}, function(err, user) {
        findThread(user);
      });
    } else if(user) {
      findThread(user);
    ...
    function findThread(user) {
      ...
      Thread.findOne({client: user._id}).exec(function(err, thread) {
        if(err) {
        ...
        // If nothing is found, create a new thread for the client
          ...
          Thread.create({client: user._id}, function(err, thread) {
